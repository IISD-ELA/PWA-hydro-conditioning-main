...
pwa.project_setup() ## For debugging, set recovery_mode to True
...
clrh_gdf = pwa.read_shapefile(filename=pwa.state.CLRH_FILENAME,
...
LIDAR_ROOT = pwa.state.HYDROCON_INTERIM_PATH if pwa.state.MULTIPLE_LIDAR_RASTERS else pwa.state.HYDROCON_RAW_PATH
...
input_lidar_crs_alnum,  CLRH_PROJ_LIDAR_FILE = pwa.project_subbasins_to_lidar(gdf=clrh_gdf,
                                   gdf_filename=pwa.state.CLRH_FILENAME,
                                   lidar_filename=pwa.state.LIDAR_FILENAME, 
...
if not pwa.state.MULTIPLE_LIDAR_RASTERS:
    ...
    # if multiple rasters were provided, they were already clipped in pwa.merge_rasters
     LIDAR_CLIPPED_FILE = LIDAR_ROOT + pwa.state.LIDAR_FILENAME
...
nhn_gdf = pwa.read_shapefile(filename=pwa.state.NHN_FILENAME,
                              directory=pwa.state.HYDROCON_RAW_PATH)
...
NHN_CLIPPED_PROJECTED_LIDAR_FILE = pwa.clip_nhn_to_watershed(nhn_filename=pwa.state.NHN_FILENAME, 
                                                        ...
                                                        culvert_filename=pwa.state.CULVERT_FILENAME)


# Generate depression depths raster
DEPRESSIONS_RASTER_FILE = pwa.gen_depressions_raster(lidar_filename=pwa.state.LIDAR_FILENAME,
...
if WANT_WETLANDS_SHAPEFILE.lower() == "yes":
    WETLAND_POLYGONS_FILE = pwa.gen_wetland_polygons(depressions_raster_file=pwa.state.DEPRESSIONS_RASTER_FILE)


# Calculate depression depths
DEPRESSION_DEPTHS_FILE = pwa.calc_depression_depths(clrh_proj_lidar_file=CLRH_PROJ_LIDAR_FILE,
                           watershed_name=pwa.state.WATERSHED_NAME,
                           depressions_raster_file=pwa.state.DEPRESSIONS_RASTER_FILE,
